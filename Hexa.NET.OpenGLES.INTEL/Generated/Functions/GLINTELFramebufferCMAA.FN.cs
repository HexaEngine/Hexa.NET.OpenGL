// ------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
// ------------------------------------------------------------------------------

using System;
using HexaGen.Runtime;
using System.Numerics;
using Hexa.NET.OpenGLES;

namespace Hexa.NET.OpenGLES.INTEL
{
	public unsafe partial class GLINTELFramebufferCMAA : GLExtension, IDisposable
	{
		public const string ExtensionName = "GL_INTEL_framebuffer_CMAA";

		public GLINTELFramebufferCMAA() : base(1)
		{
		}

		public override bool IsSupported(IGLContext context)
		{
			return context.IsExtensionSupported(ExtensionName);
		}

		protected override void InitTable(FunctionTable funcTable)
		{
			funcTable.Load(0, "glApplyFramebufferAttachmentCMAAINTEL");
		}

		public void Dispose()
		{
			funcTable.Dispose();
		}
	}
}
